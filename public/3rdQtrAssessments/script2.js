...
function getShows() {
   const showList = getShowList(); // get the object of shows
   const cat = showCat.value;  // get selected category
   ...
   for (let key in showList) {
     if (cat === "All" || showList[key].category === cat) { // show all or selected category
       rowString += "<tr>";
       rowString += "<td>" + key + "</td>";
       rowString += "<td>" + showList[key].title + "</td>";
       rowString += "<td>" + showList[key].cast + "</td>";
       rowString += "<td>" + showList[key].year + "</td>";
       rowString += "<td>" + showList[key].ratingScore() + "</td>";
       rowString += "</tr>";
     }
   }
   ...
   if (confirm(`Are you sure you want to delete all shows under "${cat}"?`)) {
     for (let key in showList) {
       if (showList[key].category === cat) {
         delete showList[key]; // remove the property from the object
       }
     }
...
function getShowList() {
  return {
    breakingBad: {
      ...
      availability: "Global",
      ratingScore: function () { return this.rating + " / " + this.imdbScore; }
    },
    theCrown: {
      title: "The Crown",
      category: "Drama",
